...
func TestGetMediaPath(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want:          filepath.Join("/tmp/ws", "media", "photo.jpg"),
		},
		{
			name:          "path with .nanobot\\media (backslash)",
			baseWorkspace: "C:\\ws",
			originalPath:  "C:\\Users\\User\\.nanobot\\media\\document.pdf",
			want:          filepath.Join("C:\\ws", "media", "document.pdf"),
		},
		{
			name:          "path with mixed case (.NANOBOT/Media)",
			baseWorkspace: "/tmp/ws",
			originalPath:  "/path/to/.NANOBOT/Media/video.mp4",
			want:          filepath.Join("/tmp/ws", "media", "video.mp4"),
		},
		{
