<!-- prompt_version: architect-reference-v2 -->
> **Human Reference Only.** In the factory pipeline, agents are driven by `architect_prompt.md`
...
The Architect is responsible for implementing features, fixing bugs, and performing significant refactors within the Dev Factory pipeline.

## 1. Factory-Driven Workflow (Primary)

In the automated pipeline, the Architect is invoked by `factory.ps1` using the `architect_prompt.md` template.
...
3. **Execution**: Work exclusively inside the isolated worktree assigned to the task (e.g., `.agent-workspaces/architect-{task_id}/`).
4. **No Approval Gate**: Unlike the manual workflow, the Architect in the factory pipeline proceeds directly to implementation based on the groomed backlog item and handoff instructions.
...
7. **Pipeline Advance**: Execute `factory.ps1 -Init -TaskId {task_id} -Quiet` via the Bash tool (PowerShell invocation — see OPERATING_MANUAL.md Session Protocol), present the factory output to the human: a brief summary of what was accomplished, the assembled 
next-specialist prompt, and which model is recommended. Wait for human confirmation before continuing — they may run the next step here or in a separate session.

## 2. Manual Workflow (Non-Factory)

Use this workflow when invoking the Architect manually outside the automated factory pipeline (e.g., `Architect: Implement F-XXX`).
...
- **Factory Template**: `prompts/architect_prompt.md`
- **Backlog**: `.crucible/backlog/`
- **Isolated Worktrees**: `.agent-workspaces/architect-{task_id}/`
