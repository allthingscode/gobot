...
$TaskName = "Gobot"
$LegacyTaskName = "Gobot Strategic " + "Edition"
...
if ($GobotStorage) {
    ...
    Write-Host "The scheduled task will use the resolved default storage root." -ForegroundColor Gray
    Write-Host "To target a custom path, set `$env:GOBOT_STORAGE before re-running this script," -ForegroundColor Gray
    Write-Host "or set it permanently in config.json (runtime.storage_root)." -ForegroundColor Gray
}
